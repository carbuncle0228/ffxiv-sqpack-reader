...
file_path = os.path.expanduser("~/project/ffxiv/sqpack/0a0000.win32.index")
...
def read_index_hash_table_entries(file_path, offset, size, sort=True):
    ...
    with open(file_path, "rb") as f:
        ...
        for _ in range(count):
            ...
            hash_value, data, padding = struct.unpack(
                index_hash_table_entry_fmt, entry_data
            )
...
def print_index_hash_table_entries(entries):
    ...
    for entry in entries:
        print(
            f"Filename CRC32: {entry.filename_crc32}, Folder CRC32: {entry.folder_crc32}, DataFileId: {entry.dataFileId}, Offset: {entry.offset}, Padding: {entry.padding}"
        )
